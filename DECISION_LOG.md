...
| `resident_holding_rules` / `non_resident_rules` | "Banxico limits and approval requirements … specific authorisation" | No general FX authorisation regime found; the real restriction is the LIC Art. 115 AML cap on US-dollar **cash** operations (cash-handling, not capital control) |
...
| `resident_holding_rules`, `non_resident_rules`, `banking_conversion_practicality` (AML cash caps) | **E1** | A1/A2 — LIC Art. 115 + SHCP/CNBV general dispositions | 2026-07-29 | exact current US-dollar cash-cap figures (2010 dispositions as later amended) to re-confirm to their current text at P8G-2b |
| `exchange_controls` — "no general authorisation/approval regime" | **E1 (bounded)** | A1 — Ley Banxico + Ley Monetaria (absence within the FX statutes examined) | 2026-07-29 | exhaustive survey of sectoral / capital-movement restrictions *outside* these statutes not performed — a bounded verification item, per CRES not asserted as an absolute |
...
**Readiness decision: Mexico HELD at `source_review_ready`.** Publication was not sought in this phase (per direction); the content is corrected and CRES-classified, but advancing to `publish_ready` is a separate future decision. The one bounded verification item (exhaustive absence of any exchange-control/capital-movement restriction outside the statutes examined) and the current US-dollar cash-cap figures are the open items for a later P8G-2b, exactly as CRES §7 prescribes (retained, refreshed, never asserted as an unsupported absolute). No governance exception.
