...
**Version transparency.** KLRI's English **Act** translation is a *reference* translation of a consolidated version (base Act No. 5550; latest amendment shown Act No. 11407, 21 Mar 2012) and is expressly non-official/non-binding; the **Regulations** translation is current to the 4 Jul 2023 amendment (Notice No. 2023-26). The 2023 FX-reform package amended chiefly the Enforcement Decree and the Regulations (e.g. raising the evidence-free overseas-remittance ceiling to USD 100,000, reducing procedural penalties); it did **not** renumber the Act's Art. 15–18 payment/capital-transaction structure relied on here. The canonical primary-law location (`law.go.kr` Act page) is retained in `source_authorities`; the KLRI viewer is the fetched, article-pinpointed source, and the two are labelled distinctly. If a stricter `law.go.kr` consolidated-text citation is later required it can be substituted without changing the findings.
...
| `summary.business` | Derived from the three closed fields + Art. 15/16/18/23 | Current transactions substantially liberalised; capital transactions (Art. 18) and specified payment methods (Art. 16) subject to transaction-specific reporting, some payments abroad to permission (Art. 15); reporting destination is a foreign exchange bank, the BOK Governor, or MOEF per delegation (Art. 23); the customs USD 10,000 border declaration, NTS bank reporting, and KoFIU AML remain separate regimes. Written only after the underlying fields were closed. |

**Correction to P8F-2 wording forced by the controlling text.** None required. The P8F-2 prose ("notification or permission … via foreign exchange banks / BOK Governor / MOEF") is consistent with the pinned articles. The only refinement: FETA uses "**report**" (advance report/declaration) as the operative term for capital transactions and methods of payment (Art. 16, 18); the field prose now cites those article numbers and the Art. 16 four-situation list, and the closed declaration/`exchange_controls`/`country_overview`/`banking_conversion_practicality` content is unchanged.
...
Status: ✅ South Korea capital-account hardening closed on the controlling FETA text (KLRI official English reference translation; article-level pinpoints for the Act, Enforcement Decree, and 2023 Regulations); zero `[HARDENING]`; South Korea advanced to `publish_ready`. Not published, not indexed, no public surface. Next step (a later, separate phase): South Korea publication release + Deployment Gate.
Verified: 2026-07-24.
