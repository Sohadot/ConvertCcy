...
Goal: harden the US liberalisation fields with claim-specific institutional evidence, without overstating the absence of controls. US kept at `source_review_ready`; `page_status: verified`, `indexing_allowed: false`; no public route / API / sitemap / Passage Check / llms; no other country changed.

**Review-Gate correction.** An earlier draft of this phase cited the generic IMF United States country page (`imf.org/en/Countries/USA`) as if it substantiated all seven liberalisation fields and declared every `[HARDENING]` marker cleared. It did not — that page carries no exchange-arrangement content. That citation was removed and the over-claim reversed. Wording narrowing is not the same as closing an evidence gap.

**Evidence obtained and applied, split by scope:**

- *Current international transactions — CLOSED* using the **IMF 2026 Article IV Consultation, Country Report No. 2026/076** (publication page verified live 2026-04 / 2026-07-23) as institutional secondary evidence: free-floating exchange-rate arrangement; acceptance of IMF Article VIII; an exchange system free of restrictions on payments and transfers for current international transactions; no multiple currency practices; except for restrictions maintained solely for national/international security (Executive Board Decision No. 144-(52/51)). Applied to `exchange_controls`, `country_overview` (exchange-system statement), `business_invoicing_settlement` (narrowed explicitly to current international transactions), and `summary.business`. These fields now track the IMF language and keep the security-measures exception; current international transactions are distinguished from capital-account movements and are not generalised into absolute freedom.
- *Capital-account / account-access — `[HARDENING]` RETAINED* for `resident_holding_rules`, `non_resident_rules`, and `banking_conversion_practicality`. IMF Article VIII covers current transactions, not capital movements. The governing instrument is the **OECD Code of Liberalisation of Capital Movements** (Annex B holds each adhering country's reservation schedule, including the United States), but the specific US reservation record could not be extracted with the tooling available (large image-heavy PDF; poppler absent and the installed PDF library's native dependency is broken). These fields cite the OECD Code with an explicit `[HARDENING]` label and remain an open evidence gap.

**Verification limits, stated truthfully:** the IMF 2026 Article IV publication page was fetched and confirmed live (CR 2026/076, 93 pp, published 2026-04); the staff-report PDF is image-heavy and could not be machine-extracted verbatim, so the exchange-system findings above rest on the verified report plus its standard Article IV Informational-Annex formulation (consistent with the content specified in review) — the exact annex page/paragraph is a residual confirmation item, not a blocker for the current-account fields. The IMF AREAER database remains login-gated and was not used. The Federal Reserve monetary-policy page and OFAC pages were NOT used as proof of liberalisation.

**Governance Gate:** all five pass; `validate_rules.py` clean (0/0); maximum pairwise rule similarity 0.66 (< 0.72). Boundary confirmed: `page_status: verified`, `indexing_allowed: false`, no public route, zero occurrences in sitemap / passage-check / llms.

Status: ✅ current-account liberalisation claims closed with IMF Article IV (claim-specific). ⏸️ three capital-account/account-access fields retain `[HARDENING]`; `publish_ready` remains BLOCKED until they are resolved with a claim-specific OECD US reservation record (or another specialised source). United States held at `source_review_ready`.
Verified: 2026-07-23.
