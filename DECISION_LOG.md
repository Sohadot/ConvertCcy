...
2. **Monetary / foreign-exchange framework — Banxico + Comisión de Cambios (live).** Mexico operates a **FREE-FLOATING** exchange regime since **22 Dec 1994**; the exchange-rate regime is set by the **Comisión de Cambios** (officials of the SHCP and Banco de México). Confirmed via the official SHCP "Anuncio de la Comisión de Cambios" (`gob.mx/shcp`, HTTP 200) and Banco de México (`banxico.org.mx`, HTTP 200). There are **no general exchange controls** on buying/selling currency or on capital movements — no Banxico permission/approval regime for carrying, holding, or transferring currency.
...
**Critical distinctions to preserve at P8G-2 (must not be conflated):** (a) Ley Aduanera Art. 9 customs border cash declaration; (b) foreign-exchange regulation (free float; no controls; Comisión de Cambios); (c) bank/intermediary reporting and USD-cash-deposit caps; (d) UIF AML/CFT reporting; (e) SAT tax/information reporting; (f) LFPIORPI domestic cash-payment limits.
...
Status: ✅ Mexico advanced `candidate_only → source_review_ready` in the intake layer, with the Ley Aduanera Art. 9 mandatory declaration (exceeding USD 10,000) and the Banxico/Comisión de Cambios free-float / no-exchange-controls framework verified live against claim-specific official sources, and the on-file OVER-statement of controls flagged for correction. Not `publish_ready`, not published, no public surface. Next step: P8G-2 publication-candidate review (correct the governed content, pin exact articles, map source_map field by field).
Verified: 2026-07-25.
